...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Powerless Over Cars"),
        actions: [
          IconButton(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          ...
          child: Stack(
            children: [
              WebViewWidget(controller: _controllers[_currentIndex]),
              if (_isLoading)
                Container(
...
// ✅ Rotating Car Loading Spinner
class RotationTransitionSpinner extends StatefulWidget {
  const RotationTransitionSpinner({super.key});

  @override
  State<RotationTransitionSpinner> createState() =>
      _RotationTransitionSpinnerState();
}
